...
def LoadConnListStr(GraphType, *args):
    return MoveArgToReturn(GraphType, args, _LoadConnListStr, 1, TStrInt64SH)

_SaveGViz = SaveGViz
def SaveGViz(Graph, *args):
    args = ConvertToSnapType(args, -1, TIntStr64H, dict)
...
def SavePajek(Graph, OutFNm, NIdColorH = None, NIdLabelH = None, EIdColorH = None):
    if NIdColorH is None:
        NIdColorH = TIntStr64H()
    if NIdLabelH is None:
        NIdLabelH = TIntStr64H()
    if EIdColorH is None:
        EIdColorH = TIntStr64H()
    args = (OutFNm, NIdColorH, NIdLabelH, EIdColorH)
    args = ConvertToSnapType(args, 1, TIntStr64H, dict)
    args = ConvertToSnapType(args, 2, TIntStr64H, dict)
    args = ConvertToSnapType(args, 3, TIntStr64H, dict)
    return _SavePajek(Graph, *args)

_GetDegCnt = GetDegCnt
def GetDegCnt(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetDegCnt, 0, TIntPr64V)

_GetInDegCnt = GetInDegCnt
def GetInDegCnt(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetInDegCnt, 0, TIntPr64V)

_GetOutDegCnt = GetOutDegCnt
def GetOutDegCnt(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetOutDegCnt, 0, TIntPr64V)        

_GetNodeInDegV = GetNodeInDegV
def GetNodeInDegV(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetNodeInDegV, 0, TIntPr64V)

_GetNodeOutDegV = GetNodeOutDegV
def GetNodeOutDegV(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetNodeOutDegV, 0, TIntPr64V)

_GetDegSeqV = GetDegSeqV
def GetDegSeqV(Graph, *args, **kwargs):
    if len(args) > 0 and type(args[0]) == TInt64V:
...
def CntEdgesToSet(Graph, *args):
    args = ConvertToSnapType(args, 1, TInt64Set, set)
    return _CntEdgesToSet(Graph, *args)

_DelNodes = DelNodes
def DelNodes(Graph, *args):
    args = ConvertToSnapType(args, 0, TInt64V, list)
...
def GetSccSzCnt(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetSccSzCnt, 0, TIntPr64V)  
...
def GetWccSzCnt(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetWccSzCnt, 0, TIntPr64V)

_GetNodeWcc = GetNodeWcc
def GetNodeWcc(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetNodeWcc, 1, TInt64V)
...
def Get1CnComSzCnt(Graph, *args):
    return MoveArgToReturn(Graph, args, _Get1CnComSzCnt, 0, TIntPr64V)
...
def GetBiConSzCnt(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetBiConSzCnt, 0, TIntPr64V)

_GetArtPoints = GetArtPoints
def GetArtPoints(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetArtPoints, 0, TInt64V)

_GetEdgeBridges = GetEdgeBridges
def GetEdgeBridges(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetEdgeBridges, 0, TIntPr64V)

_GetNodesAtHop = GetNodesAtHop
def GetNodesAtHop(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetNodesAtHop, 2, TInt64V)

_GetNodesAtHops = GetNodesAtHops
def GetNodesAtHops(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetNodesAtHops, 1, TIntPr64V)
...
def GetBetweennessCentr(Graph, *args):
    if len(args)>=1 and type(args[0]) == TIntFlt64H and type(args[1]) == TIntPrFlt64H:
        #Backward compatibility
...
def GetPageRank(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetPageRank, 0, TIntFlt64H)

_GetHits = GetHits
def GetHits(Graph, *args, **kwargs):
    if len(args) >= 2 and (type(args[0]) == TIntFlt64H) and (type(args[1]) == TIntFlt64H):
        #Backward compatibility
...
def GetEigenVectorCentr(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetEigenVectorCentr, 0, TIntFlt64H)
...
def GetClustCf(Graph, *args, **kwargs):
    if len(args) == 2 and (type(args[0]) == TFltPr64V):
        #Backward compatibility
        return _GetClustCf(Graph, *args)
    if len(args) == 1 and (type(args[0]) == int or type(args[0]) == TFltPr64V):
        #Backward compatibility
        return _GetClustCf(Graph, *args)
...
def GetClustCfAll(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetClustCfAll, 0, TFltPr64V)
...
def GetTriadParticip(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetTriadParticip, 0, TIntPr64V)

_GetKCoreNodes = GetKCoreNodes
def GetKCoreNodes(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetKCoreNodes, 0, TIntPr64V)

_GetKCoreEdges = GetKCoreEdges
def GetKCoreEdges(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetKCoreEdges, 0, TIntPr64V)

_GetEigVals = GetEigVals
def GetEigVals(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetEigVals, 1, TFlt64V)

_GetSngVals = GetSngVals
def GetSngVals(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetSngVals, 1, TFlt64V)

_GetInvParticipRat = GetInvParticipRat
def GetInvParticipRat(Graph, *args):
    return MoveArgToReturn(Graph, args, _GetInvParticipRat, 2, TFltPr64V)
...
def GetSngVecs(Graph, SngVecs):
    SngValV = TFlt64V()
    LeftSV = TFlt64VFlt64V()
    RightSV = TFlt64VFlt64V()
...
def GetBfsEffDiam(Graph, *args):
    args = ConvertToSnapType(args, 1, TInt64V, list)
...
def GetLen2Paths(Graph, *args):
    ...
    return MoveArgToReturn(Graph, args, _GetLen2Paths, 2, TInt64V)
